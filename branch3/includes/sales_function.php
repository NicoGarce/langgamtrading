...
class Sales{
    ...
    public function importSale()
    {
        ...
        if (isset($_POST["import"])) {
            ...
            foreach ($reader as $key => $row) {
                ...
                $insertQuery = "INSERT INTO branch3_sales (customer_name, order_date, order_time, order_list, total_cost, pay_method, order_status, payment_status, contact_info, order_type, user_id, salesperson, shipping_details, date_complete, time_complete) 
    ...
    public function getTop10MostBoughtItems()
    {
        $store = new Langgam();
        $conn = $store->openConnection();
        $stmt = $conn->prepare("SELECT * FROM branch3_sales WHERE order_status NOT IN ('Cancelled', 'Returned', 'Refunded')");
        $stmt->execute();
        $results = $stmt->fetchAll(PDO::FETCH_OBJ);
