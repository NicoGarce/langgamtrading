...
$salesCountStmt = $conn->prepare('SELECT COUNT(*) AS sales_count FROM branch3_sales');
$salesCountStmt->execute();
$salesCount = $salesCountStmt->fetchColumn();

$cancelledOrdersCountStmt = $conn->prepare('SELECT COUNT(*) AS cancelled_orders_count FROM branch3_orders WHERE order_status = :Cancelled');
...
$returnedOrdersCountStmt = $conn->prepare('SELECT COUNT(*) AS returned_orders_count FROM branch3_orders WHERE order_status = :Returned');
...
$refundedOrdersCountStmt = $conn->prepare('SELECT COUNT(*) AS refunded_orders_count FROM branch3_orders WHERE order_status = :Refunded');
