    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        registrationForm.addEventListener('submit', async (event) => {
            ...
            if (password !== confirm) {
                ...
                const isUsernameAvailable = await checkAvailability('/langgamtrading/branch3/includes/check_username.php', `username=${username}`, usernameMessage);
                const isEmailAvailable = await checkAvailability('/langgamtrading/branch3/includes/check_email.php', `email=${email}`, emailMessage);
                const isMobileAvailable = await checkAvailability('/langgamtrading/branch3/includes/check_mobile.php', `mobile=${mobile}`, mobileMessage);
        ...
        usernameInput.addEventListener('input', async () => {
            ...
            if (username.length >= 3) {
                await checkAvailability('/langgamtrading/branch3/includes/check_username.php', `username=${username}`, usernameMessage);
            } else {
        ...
        emailInput.addEventListener('input', async () => {
            const email = emailInput.value.trim();
            if (email.length >= 3) {
                await checkAvailability('/langgamtrading/branch3/includes/check_email.php', `email=${email}`, emailMessage);
            } else {
        ...
        mobileInput.addEventListener('input', async () => {
            const mobile = mobileInput.value.trim();
            if (mobile.length >= 3) {
                await checkAvailability('/langgamtrading/branch3/includes/check_mobile.php', `mobile=${mobile}`, mobileMessage);
            } else {
        ...
        confirmInput.addEventListener('input', async () => {
            ...
            if (confirm !== '') {
                const response = await checkConfirmation('/langgamtrading/branch3/includes/check_confirmation.php', `password=${password}&confirm=${confirm}`);
