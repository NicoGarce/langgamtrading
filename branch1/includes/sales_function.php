...
class Sales
{
    public function get_sales()
    {
        ...
        $stmt = $conn->prepare("SELECT * FROM branch1_sales;");
        $stmt->execute();
    ...
    public function get_sale_by_id($sale_id)
    {
        $store = new Langgam();
        $conn = $store->openConnection();
        $stmt = $conn->prepare("SELECT * FROM branch1_sales WHERE sale_id = :sale_id");
    ...
    public function importSale()
    {
        ...
        if (isset($_POST["import"])) {
            ...
            foreach ($reader as $key => $row) {
                ...
                $insertQuery = "INSERT INTO branch1_sales (customer_name, order_date, order_time, order_list, total_cost, pay_method, order_status, payment_status, contact_info, order_type, user_id, salesperson, shipping_details, date_complete, time_complete) 
    ...
    public function getTop10MostBoughtItems()
    {
        $store = new Langgam();
        $conn = $store->openConnection();
        $stmt = $conn->prepare("SELECT * FROM branch1_sales WHERE order_status NOT IN ('Cancelled', 'Returned', 'Refunded')");
        $stmt->execute();
        $results = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    public function delete_sale()
    {
        ...
        if (isset($_REQUEST['delete'])) {
            ...
            $sql = "DELETE FROM branch1_sales where sale_id =:sale_id";
