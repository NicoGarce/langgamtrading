...
// Query to get the count of sales from branch1_sales
$salesCountStmt = $conn->prepare('SELECT COUNT(*) AS sales_count FROM branch1_sales');
$salesCountStmt->execute();
$salesCount = $salesCountStmt->fetchColumn();

// Query to get the count of cancelled orders from branch1_orders
$cancelledOrdersCountStmt = $conn->prepare('SELECT COUNT(*) AS cancelled_orders_count FROM branch1_orders WHERE order_status = :Cancelled');
...
// Query to get the count of returned orders from branch1_orders
$returnedOrdersCountStmt = $conn->prepare('SELECT COUNT(*) AS returned_orders_count FROM branch1_orders WHERE order_status = :Returned');
...
// Query to get the count of refunded orders from branch1_orders
$refundedOrdersCountStmt = $conn->prepare('SELECT COUNT(*) AS refunded_orders_count FROM branch1_orders WHERE order_status = :Refunded');
