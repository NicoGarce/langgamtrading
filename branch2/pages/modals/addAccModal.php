    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        registrationForm.addEventListener('submit', async (event) => {
            ...
            if (password !== confirm) {
                ...
                const isMobileAvailable = await checkAvailability('/langgamtrading/branch2/includes/check_mobile.php', `mobile=${mobile}`, mobileMessage);
        ...
        mobileInput.addEventListener('input', async () => {
            const mobile = mobileInput.value.trim();
            if (mobile.length >= 3) {
                await checkAvailability('/langgamtrading/branch2/includes/check_mobile.php', `mobile=${mobile}`, mobileMessage);
            } else {
