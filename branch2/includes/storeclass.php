...
class Langgam
{
                ...
                if ($width === $height) {
                    if ($file_size < 5120000) {
                        if (move_uploaded_file($file_temp, $loc.$location)) {
                            try {
                                $conn = $this->openConnection();
                                $stmt = $conn->prepare("UPDATE branch2_users SET photo = :location WHERE ID = :userid");
    ...
    {
        $conn = $this->openConnection();
        $stmt = $conn->prepare("SELECT * FROM branch2_inventory");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }
    ...
    public function add_product()
    {
        ...
        if (isset($_POST['add_product'])) {
            ...
            $supplier_name = "";
            $stmt = $pdo->prepare("SELECT supplier_name FROM branch2_suppliers WHERE supplier_id = :supplier_id");
            ...
            $sql = "INSERT INTO branch2_inventory 
